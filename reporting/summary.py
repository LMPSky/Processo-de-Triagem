...
import pandas as pd
from classifiers.common import sanitize_sheet_name, rename_output_columns, truncate_text_column, word_frequency
...
def export_summary_excel(
    ...
    with pd.ExcelWriter(xlsx_path, engine="openpyxl") as writer:
        ...
        if len(sem_match_civel) > 0:
            ...
            cols = [c for c in [
                "cnj", "_texto", "Data da Publicação", "Data da Captura", "_cliente", "Tribunal", "Sistema",
...
def export_trabalhista_analysis(
    summary_path: Path,
    sem_match_trab: pd.DataFrame,
) -> None:
    """
    Exporta análise detalhada de processos trabalhistas NÃO CATEGORIZADOS.
    Útil para encontrar padrões de texto enganosos.
    """
    if len(sem_match_trab) == 0:
        return

    xlsx_path = summary_path.with_suffix(".xlsx")

    with pd.ExcelWriter(xlsx_path, engine="openpyxl", mode="a", if_sheet_exists="replace") as writer:
        trab_bruto = truncate_text_column(rename_output_columns(sem_match_trab.copy()))
        cols = [c for c in [
            "cnj", "_texto", "Data da Publicação", "Data da Captura", "_cliente", "Tribunal", "Sistema"
        ] if c in trab_bruto.columns]
        trab_bruto[cols].to_excel(
            writer,
            sheet_name=sanitize_sheet_name("Trabalhista Bruto"),
            index=False
        )

        freq_trab = word_frequency(sem_match_trab["_texto"], top_n=100)
        freq_trab.to_excel(
            writer,
            sheet_name=sanitize_sheet_name("Palavras Trab"),
            index=False
        )

        if "_tribunal" in sem_match_trab.columns:
            ranking_tribunal_trab = (
                sem_match_trab["_tribunal"].fillna("").astype(str)
                .value_counts()
                .head(50)
                .reset_index()
            )
            ranking_tribunal_trab.columns = ["Tribunal", "Quantidade"]
            ranking_tribunal_trab.to_excel(
                writer,
                sheet_name=sanitize_sheet_name("Ranking Tribunal Trab"),
                index=False
            )

        ranking_cliente_trab = (
            sem_match_trab["_cliente"].fillna("").astype(str)
            .value_counts()
            .head(50)
            .reset_index()
        )
        ranking_cliente_trab.columns = ["Cliente", "Quantidade"]
        ranking_cliente_trab.to_excel(
            writer,
            sheet_name=sanitize_sheet_name("Ranking Cliente Trab"),
            index=False
        )

        if "_sistema" in sem_match_trab.columns:
            ranking_sistema_trab = (
                sem_match_trab["_sistema"].fillna("").astype(str)
                .value_counts()
                .head(50)
                .reset_index()
            )
            ranking_sistema_trab.columns = ["Sistema", "Quantidade"]
            ranking_sistema_trab.to_excel(
                writer,
                sheet_name=sanitize_sheet_name("Ranking Sistema Trab"),
                index=False
            )

        if "_tribunal" in sem_match_trab.columns and "_sistema" in sem_match_trab.columns:
            df_ts = sem_match_trab.copy()
            df_ts["_tribunal"] = df_ts["_tribunal"].fillna("").astype(str)
            df_ts["_sistema"] = df_ts["_sistema"].fillna("").astype(str)
            ...
            ts_counts.to_excel(
                writer,
                sheet_name=sanitize_sheet_name("Tribunal_Sistema Trab"),
                index=False
            )